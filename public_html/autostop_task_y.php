...
<?php
	...
	$checkdate_ts = mktime (0,0,0, date("m"), date("d") - 1, date("Y"));
	$cdate = getdate($checkdate_ts); 
	$cmonth = $cdate['mon']; 
	$cday   = $cdate['mday']; 
	$cyear  = $cdate['year']; 
	$checkdate = date("Y-m-d", $checkdate_ts);
    ...
    //echo $sql."<br>";
    $message2 = "<html><head><title>Monitor: List of users who forgot to stop their tasks - $checkdate</title></head>";
    $message2 .= "<body style=\"background-color: #FFFFFF; color: #000000; font-family: Tahoma,Verdana,Helvetica; font-size: 10pt\">";
    $message2 .= "<center><b>List of users who forgot to stop their tasks - $checkdate</b></center><br>";
    ...
    if ($db->next_record()) {
	    do {
	    	...
	    	if ($started_stamp < $stamp_18h) {
	    		...
	    		$report_date = date("Y-m-d", $checkdate_ts) . " 18:00:00";
    		}
    		else {
	    		$spent_hours = 1/60;
...
*/			
	    	...
	    	$subj = "Monitor: Your task '$task_title' wasn't closed in time - $checkdate";
	    	$message = "<html><head><title>Monitor: Your task '$task_title' wasn't closed in time - $checkdate</title></head>";
		...
		//$to = "eugene@viart.com.ua";
		$subj = "Monitor: List of users who forgot to stop their tasks - $checkdate";
