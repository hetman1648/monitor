<?php
	...
	*/
	function CheckImage()
	{
		...
		if (!$result)
		{
			$message = "Image not found at " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
	...
	function GetRemoteImage()
	{
		...
		$message = "Start geting remote image " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
		ReportMessage($message);
		...
		$message = "Start parse web page " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
		ReportMessage($message);
		...
		if (strlen($content) > 0)
		{
			if (strpos($content, "Страница не найдена") === false)
			{
				...
				if (preg_match_all($reg,$content,$out))
				{
					...
					$message = "Founded " . sizeof($image_array) . " images at " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
					ReportMessage($message);
					...
					foreach ($image_array as $im_url)
					{
						...
						if (!$image)
						{
							$message = "Cann`t download image(imagecreatefromjpeg) from " . $timg . " at " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
							ReportMessage($message);
							continue;
						}
						$background_color = @imagecolorallocate($image_p, 255, 255, 255);
						$r = @imagefilledrectangle($image_p,0,0,$wsize,$hsize_new,$background_color);
						
						if (!$r)
						{
							$message = "Cann`t fill image(imagefilledrectangle) from " . $timg . " at " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
							ReportMessage($message);
							continue;
						}
						
						//@imagecopyresampled($image_p, $image, 0, $white_line_px, 0, 0, $wsize, $hsize_new, $wsize, $hsize);
						$r = @imagecopy($image_p, $image,0,$white_line,0,0,$wsize, $hsize);
						
						if (!$r)
						{
							$message = "Cann`t copy image(imagecopy) from " . $timg . " at " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
							ReportMessage($message);
							continue;
						}
						...
						if (!$r)
						{
							$message = "Cann`t create image(imagejpeg) from " . $timg . " at " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
							ReportMessage($message);
							continue;
						}
					...
					if ($count > 1)
					{
						if (!DeleteOldImage())
						{
							$message = "Cann`t delete old image at " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
							ReportMessage($message);
						}
					}
				}
				else
				{
					...
					if (preg_match_all($reg,$content,$out))
					{
						if (sizeof($out) == 4)
						{
							...
							$r = @imagecopyresampled($image_p, $image, 0, $white_line, 0, 0, $wsize, $hsize_new, $wsize, $hsize);
					...
					else
					{
						...
						$message = "Not found any images on the page at " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
						ReportMessage($message);
						CopyOldImages();
					}
				}
			...
			{
				///Copy old images as new
				$message = "Not found web page at " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
				ReportMessage($message);
				CopyOldImages();
			}
		}
		...
		{
			$message = "Not founded anything " . date("H:i:s d-m-Y") . "[" . __LINE__ . "]\n";
			ReportMessage($message);
		}
	}
			...
			while (false !== ($file = readdir($handle)))
			{
					...
					strpos($path_parts['basename'],$current_name) === false)
				{
					...
					if (preg_match($reg,$path_parts['basename'],$out))
					{
						...
						if (($mon . $day) != ($m . $d))
						{
							$result = @unlink($image_folder . $file);
						}
					}
				}
			}
	...
	function ReportMessage($message = "")
	{
		if (strlen($message) > 0)
		{
			$mail_head  = "From: kalendar@viart.com.ua" . "\r\n";
			$mail_head .= "Content-type: text/html; charset=iso-8859-1" . "\r\n";
			@mail("sanuch@viart.com.ua","Kalendar Script",$message, $mail_head);
		}
	}
?>