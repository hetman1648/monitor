...
function setInitialDate() {
    ...
    // (THIS ROUTINE DOES *NOT* CATCH ALL DATE FORMATS, IF YOU NEED TO PARSE A CUSTOM DATE FORMAT, DO IT HERE)
			var calYear;
			var calMonth;
		if(calDateFormat == "YY-MM-DD" && inDate.length == 10)
		{
			var inDateYear = inDate.substring(0,4);
			var inDateMonth = inDate.substring(5,7) - 1;
			var inDateDay = inDate.substring(8,10);

	    calDate = new Date(inDateYear, inDateMonth, inDateDay);
		}
		else if(calDateFormat == "DD-MM-YY" && inDate.length == 10)
		{
			var inDateDay = inDate.substring(0,2);
			var inDateMonth = inDate.substring(3,5) - 1;
			var inDateYear = inDate.substring(6,10);
...
function returnDate(inDay)
{
  ...
  xxx = xxx.replace("DD", day);
	
	month = makeTwoDigit(month);
	
	xxx = xxx.replace("MM", month);

	xxx = xxx.replace("YY", year);

    calDateField.value = xxx;
