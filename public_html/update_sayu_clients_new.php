...
$db_web = new DB_Sql();
$db_web->Database = DATABASE_NAME;
$db_web->User     = DATABASE_USER;
$db_web->Password = DATABASE_PASSWORD;
$db_web->Host     = DATABASE_HOST;
...
$db->query("DROP TABLE IF EXISTS tsayu_clients");
	...
	if ($db->num_rows()) {
	    ...
	    $sql = "SELECT	cap.client_id		AS sayu_user_id,
	    				...
	    				c.client_company	AS client_company,
	    				IFNULL(cap.account_mcc,0) AS account_mcc,
						IF(c.is_current_client=-1,1,0)	AS is_active,
	    ...
	    $update_old = false;
	    while ($db2->next_record()) {
			$sayu_user_id = $db2->f("sayu_user_id");
			$sql = "SELECT * FROM clients WHERE sayu_user_id=".ToSQLO($sayu_user_id,"integer",false,false);
	    	$db->query($sql,__FILE__,__LINE__);
	    	$db->next_record();
	    	if ((strcasecmp($db2->f("client_name"),$db->f("client_name")) !=0 && strlen($db2->f("client_name"))>1) ||
	    		...
	    		(strcasecmp($db2->f("google_accounts_emails"),$db->f("google_accounts_emails")) !=0 && strlen($db2->f("google_accounts_emails"))>1) )
			{
					$update_old = true;
	    			$sql = "UPDATE clients
								SET	client_name		='".mysql_escape_string($db2->f("client_name"))."',
									...
									is_active		=".ToSQLO($db->f("is_active"),"integer",false,false).",
									google_accounts_emails =".ToSQLO($db2->f("google_accounts_emails"),"string",true,true)."
								WHERE	sayu_user_id=".ToSQLO($db->f("sayu_user_id"),"integer",false,false);
					$message .= $db->f("client_name")."(".$db->f("sayu_user_id").")[".$db->f("sayu_user_id")."]\r\n";
					$message .= $dbo->f("client_name")."(".$dbo->f("sayu_user_id").") [".$dbo->f("sayu_user_id")."]\n";
					...
					$message .= "google_accounts_emails =".ToSQLO($db2->f("google_accounts_emails"),"string",true,true)."  \n";
					$message .= "user_id=".ToSQLO($sayu_user_id,"integer",false)."\r\n";
					/**/
					$db->query($sql,__FILE__,__LINE__);
			} else {
				$sql  = " UPDATE clients SET is_active =".ToSQLO($db->f("is_active"),"integer",false,false);
				$sql .= " WHERE	sayu_user_id=".ToSQLO($db->f("sayu_user_id"),"integer",false,false);
				$db->query($sql,__FILE__,__LINE__);
			}
		}
	}
	...
	while ($db2->next_record()) {
		...
		$client_type	= $db2->Record['client_type'];
		$web_address	= str_replace("http://","",$db2->Record['web_address']);
		$is_active		= $db2->Record['is_active'];
		$account_mcc	= $db2->Record['account_mcc'];
		...
		$sql_google = " 1 ";
		if (sizeof($emails) > 1) {
			$sql_email .= " AND (client_email LIKE '%".array_shift($emails)."%' ";
			foreach($emails as $k => $v) {
				$sql_email .= " OR client_email LIKE '%".$v."%' ";
			}
			$sql_email .= " ) ";
		} elseif (sizeof($emails) == 1) {
			if (is_array($emails)) { $sql_email  = " client_email LIKE '%".array_shift($emails)."%' ";}
			else { $sql_email  = " client_email LIKE '%".$emails."%' ";}
		}

		if (sizeof($googles) > 1) {
			$sql_google.= " AND ( google_id LIKE '%".array_shift($googles)."%' ";
			foreach ($googles as $k => $v) {
				$sql_google .= " OR google_id LIKE '%".$v."%' ";
			}
			$sql_google .= " ) ";
		} elseif (sizeof($googles) == 1) {
			if (is_array($googles)) { $sql_google  = " client_email LIKE '%".array_shift($googles)."%' ";}
			else { $sql_google  = " client_email LIKE '%".$googles."%' ";}
		}
		
		$sql_company = "";
		if ($client_company) {
			$sql_company .= " OR client_company LIKE " . ToSQL("%".addslashes($client_company)."%","string");
		}

		$sql = "SELECT * FROM clients WHERE client_type=2 AND (" . $sql_email . " OR " . $sql_google . $sql_company . " ) ";
		$db->query($sql);
		if ($db->num_rows() == 1) {
			$update_old = true;
			$db->next_record();
			...
			$sql_web = "SELECT * FROM clients_sites WHERE client_id = " . ToSQL($client_id,"number");
			$db_web->query($sql_web,__FILE__,__LINE__);
			if ($db_web->next_record()) {
				$sql_web = "UPDATE clients_sites SET web_address = " . ToSQL($web_address,"string") . " WHERE client_id = " . ToSQL($client_id,"number");
			} else {
				$sql_web = "INSERT INTO clients_sites SET
								client_id	= " . ToSQL($client_id,"number") . ",
		...
		elseif ($db->num_rows() == 0) {
			$sql = "INSERT INTO clients
					SET sayu_user_id	=".ToSQLO($sayu_user_id,"integer",false).",
						client_name		=".ToSQLO(mysql_escape_string($client_name),"string",true,true).",
						client_email	=".ToSQLO($client_email,"string",true,true).",
						date_added		=".ToSQLO($date_added,"date").",
						client_company	=".ToSQLO(mysql_escape_string($client_company),"string",true,true).",
						client_type		=".ToSQLO($client_type,"integer",false).",
						is_active		=".ToSQLO($is_active,"integer",false).",
						account_mcc		=".ToSQLO($account_mcc,"string",true,true).",
						google_id		='".trim(ereg_replace("[- ]","",$db2->Record['google_id']))."',
						google_accounts_emails = ".ToSQLO($google_accounts_emails,"string",true,true);
			$update_new = true;
			...
			$message .= "google_accounts_emails =".ToSQLO($google_accounts_emails,"string",true,true)."\r\n ";
		}			
		else {}//many clients :(
		if ($update_new) {
			$sql_web = "SELECT LAST_INSERT_ID()";
			$db->query($sql_web,__FILE__,__LINE__);
			$db->next_record();
			$client_id = $db->Record[0];
			if ($client_id>0) {
				$sql_web = "INSERT INTO clients_sites SET
								client_id	= " . ToSQL($client_id,"number") . ",
								web_address	= " . ToSQL($web_address,"string") . ",
	...
	if ($update_new || $update_old) {
		mail("sanuch@viart.com.ua","Update clients",$message);
	}
	/**/
