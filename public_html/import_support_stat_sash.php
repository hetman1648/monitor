...
<?php
	...
	$advanced_url = "http://www.viart.com/support_reports_sash.php";
	...
	/*/
	$db->query("DELETE FROM support_import_sash");
	$db->query("DELETE FROM support_reports_sash");
	/**/
	$initial = getmicrotime();
	$file_log = "./import_for_sanuch_new.log";
			...
			if ($ch) {
				...
				if (strlen($return_value)) {
					...
					if (is_array($report_array) && sizeof($report_array)) {
						if (!$last_update_id) {
							$sql = "INSERT INTO support_import_sash SET update_date = '" . date("Y-m-d H:i:s",$last_update) ."' ";
							$db->query($sql);
							...
							$sql  = " SELECT * FROM support_import_sash ";
							...
							$db->query($sql);
							if ($db->next_record()) {
								$sql = "UPDATE support_import_sash SET update_date = '" . date("Y-m-d H:i:s",$last_update) ."' WHERE import_id = " . ToSQL($last_update_id,"number");
								$db->query($sql);
								echo "(" . __LINE__ . "): " . $sql . "\r\n";
								$sql = "DELETE FROM support_reports_sash WHERE import_id = " . ToSQL($last_update_id,"number");
								$db->query($sql);
							} else {
								$sql = "INSERT INTO support_import_sash SET update_date = '" . date("Y-m-d H:i:s",$last_update) ."' ";
								$db->query($sql);
						...
						foreach ($report_array as $helpdesk_user_id => $reports) {
							if (!isset($support_users_ids[$helpdesk_user_id])) {
								...
								foreach($reports as $type => $count) {
									...
									$type_id = $type + 1;
									$sql  = " INSERT INTO support_reports_sash ";
