	...
	function set_categories($parent_id, $level, $columns, $top_id, $image_type = 1)
	{
		...
		for ($i = 0; $i < sizeof($subs); $i++)
		{
			...
			if ($level > 0) {
				...
				$t->set_var("category_tr_class", "subCategory");
			} else {
			...
			$t->set_var("category_name", $tree_images . $category_name);
			if ($is_rss) {
		...
		if ($convert_links) {
			if (preg_match_all("/([='\"]?)(https?:\\/\\/[\w\d_\-\+\\%\.\?\&\#\~\=\\/]+)(['\"]?)/i", $message, $matches)) {
				...
				for ($p = 0; $p < sizeof($matches[0]); $p++) {
					...
					if ($before_link != "=" && $before_link != "\"" && $before_link != "'") {
				...
				foreach ($links as $link => $link_name) {
					$message = str_replace($link, "<a href=\"" . $link . "\" target=\"_blank\">" . $link_name . "</a>", $message);
				}
			}
		}
		...
		if ($db->next_record())
		{
			...
			if ($registration_last_step < $registration_total_steps) {
				...
				$user_info = array(
					...
					"login" => $login, "nickname" => $nickname, "name" => $user_name,
